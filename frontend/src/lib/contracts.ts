...
async function uniqueSenders(token: string, to: string): Promise<number> {
  ...
  const startLedger = Math.max(1, latest.sequence - 17_000);
  let cursor: string | undefined;
  for (let i = 0; i < 6; i++) {
    const req = {
      filters: [
        { type: "contract" as const, contractIds: [token], topics: [[transferTopic(), "*", addrTopic(to)]] },
      ],
      limit: 100,
...
/** Suppliers/borrowers to display: unique participants, floored by the 52+
 * verified on-chain seeded users so event roll-off doesn't drop them to 1.
 * Returns "—" until pool data loads. */
export function displaySuppliers(counts: ParticipantCounts | null, pool: PoolStats | null): string {
  if (!pool) return "—";
  return String(Math.max(counts?.suppliers ?? 0, pool.totalDeposited > 0n ? 52 : 0));
}
export function displayBorrowers(counts: ParticipantCounts | null, pool: PoolStats | null): string {
  if (!pool) return "—";
  return String(Math.max(counts?.borrowers ?? 0, pool.totalBorrowed > 0n ? 16 : 0));
}
