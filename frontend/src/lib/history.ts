...
const HKEY = "verdex:marketHistory";
...
const cbKey = (addr: string) => `verdex:costBasis:${addr}`;
...
const ipKey = (addr: string) => `verdex:interestPaid:${addr}`;
...
const actKey = (addr: string) => `verdex:activity:${addr}`;
